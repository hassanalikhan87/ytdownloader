...
async function downloadVideo() {
  ...
  // after success
  updateProgress('video-progress', 100);
...
//   status.textContent = '⏳ Downloading playlist...';
  ...
  status.textContent = '⏳ Downloading playlist...';
  showProgress('playlist-progress');

  // after success
  updateProgress('playlist-progress', 100);
  ...
  try {
    ...
    if (data.status === 'success') {
      let links = data.videos
        .map((v) => `<li><a href="/get_file/${v.filename}">${v.title}</a></li>`)
        .join('');
...
function showProgress(id) {
  ...
  fill.style.width = '30%'; // initial loading state
}
